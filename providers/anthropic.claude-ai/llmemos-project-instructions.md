...
7. `llmemos-publish` automatically injects `/rename <title>` into the active Claude
   session after publishing, keeping the local session name in sync with the memo title.
   Pass `--no-rename` to skip this step.
